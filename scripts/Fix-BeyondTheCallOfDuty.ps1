...
.SYNOPSIS
    ...
    Commandos: Beyond the Call of Duty, and makes it behave on Windows 10/11.

.DESCRIPTION
    The 2016 Steam build of Commandos: Beyond the Call of Duty ships with four hard-coded
    ...
    Localised text inside the game archive (Czech, German, ... ) is preserved: the
    only strings touched are the four resolution captions, rewritten in place.
...
$GAME = @{
    ...
    SteamAppId   = 6810
    InstallDir   = 'Commandos Beyond the Call of Duty'
    ExeName      = 'coman_mp.exe'
    ArchiveName  = 'WAR_MP.DIR'
    DocsDir      = 'Commandos - Beyond the call of duty'   # under \Documents
    KnownExeSize = 4512768                                 # 2016 Steam build
    ...
    Patterns     = @(
        @{ Name = 'video mode setters';        Spec = '68 {H} 68 {W}';                                     Expect = 3 }
        @{ Name = 'slot -> resolution table';  Spec = 'C7 86 64 02 00 00 {W} C7 86 68 02 00 00 {H}';        Expect = 1 }
        @{ Name = 'resolution -> slot map';    Spec = '3D {W} 75 05 B9 04 00 00 00';                        Expect = 1 }
        @{ Name = 'resolution -> menu bitmap'; Spec = '3D {W} 74 09 C7 46 5C 00 00 00 00';                  Expect = 1 }
    )
}
...
#  the better part of a minute on a 4.5 MB executable.
# ---------------------------------------------------------------------------
...
#  other offset in the archive stays valid - which is what keeps a translated
#  GLOBAL.STR (Czech, German, ...) intact.
# ---------------------------------------------------------------------------
$ENTRY_SIZE = 44
...
if ($sourceW) {
    ...
    foreach ($p in $GAME.Patterns) {
        ...
        $total += $hits.Count
        Write-Step ("{0,-26} {1} site(s) at {2}" -f $p.Name, $hits.Count, (($hits | ForEach-Object { '0x{0:x}' -f $_ }) -join ', '))
    }
...
if (-not (Test-Path $bmpSrc) -or -not (Test-Path $wadSrc)) {
    ...
    Write-Warn2 'The game needs both: without a matching .WAD it exits when the mission loads.'
...
Write-Host '   - One map in this game (the smallest one) is narrower than a 1920 px'
Write-Host '     screen, so a black strip appears on the right. Press + to zoom in and'
Write-Host '     it fills the screen. This is a limit of the 1998 engine, not of the patch.'
